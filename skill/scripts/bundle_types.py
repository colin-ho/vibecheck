...
class TokenCounts(BaseModel):
    ...
    input: conint(ge=0)
    output: conint(ge=0)
    cached: conint(ge=0) = Field(
        ..., description='Cache read tokens (served from cache)'
    )
    cacheCreation: Optional[conint(ge=0)] = Field(
        None, description='Cache creation tokens (written to cache)'
    )
