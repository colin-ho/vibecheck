...
export const personas: Record<string, PersonaDefinition> = {
  ...
  'token-titan': {
    ...
    description: 'You\'ve processed more tokens than most people have typed characters. The models know you by name.',
    color: '#ffd700',
  ...
  'prompt-whisperer': {
    ...
    description: 'Your prompts are surgical. High efficiency, low waste. You speak and Claude understands.',
    color: '#00ff88',
  ...
  'tool-master': {
    ...
    description: 'Read, Write, Bash, Edit, Grep... you\'ve mastered them all. A true multi-tool operator.',
    color: '#00d4ff',
  ...
  'mcp-pioneer': {
    ...
    description: 'Custom integrations, custom tools. You\'re not just using Claude, you\'re augmenting it.',
    color: '#a855f7',
  ...
  'vibe-coder': {
    ...
    description: 'You don\'t read error messages, you just retry and hope. Stack traces? Never heard of \'em. Your debugging strategy is "idk just fix it".',
    color: '#ff6b6b',
  ...
  'copy-paste-warrior': {
    ...
    description: 'Why understand code when you can just copy it? Your clipboard has seen things. Typing is for people who don\'t know where the good code lives.',
    color: '#4ecdc4',
  ...
  '3am-demon': {
    ...
    description: 'Sleep is for the weak. Coffee is your co-pilot. The quietest hours of the night are when you truly come alive. Your circadian rhythm is a suggestion you\'ve ignored.',
    color: '#9b59b6',
  ...
  'polite-menace': {
    ...
    description: 'You\'re suspiciously polite for someone writing code at this hour. Every "please" and "thank you" hides the chaos you\'re about to unleash. Manners maketh the menace.',
    color: '#2ecc71',
  ...
  'caps-lock-commander': {
    ...
    description: 'CALM DOWN. IT\'S JUST CODE. Your caps lock key has seen some things. We can hear you through the screen. Maybe try decaf?',
    color: '#e74c3c',
  ...
  'essay-writer': {
    ...
    description: 'Your average prompt could be a Medium article. You\'ve written more context than code. Claude needs a coffee break after reading your messages.',
    color: '#3498db',
  ...
  'one-word-wonder': {
    ...
    description: 'Your entire vocabulary fits on a Post-it note. Claude has to read minds because you certainly aren\'t providing context. Efficiency king or just lazy?',
    color: '#95a5a6',
  ...
  'css-struggler': {
    ...
    description: 'Flexbox? More like flex-nope. Your relationship with CSS is... complicated. Every centered div is a small victory. We believe in you.',
    color: '#9b59b6',
  ...
  'squirrel-brain': {
    ...
    description: 'You asked Claude to do something, then interrupted. Multiple times. Your attention span is measured in milliseconds. Ooh, what\'s that shiny thing?',
    color: '#f39c12',
  ...
  'bash-berserker': {
    ...
    description: 'Over 60% Bash usage. When in doubt, run a command. Documentation is for the weak.',
    color: '#fb923c',
  ...
  'context-amnesiac': {
    ...
    description: 'Low cache hit rate detected. You\'re paying full price for the same context. Every. Single. Time.',
    color: '#94a3b8',
  ...
  'agent-interrupter': {
    ...
    description: 'You asked Claude to do something, then interrupted. Multiple times. Claude forgives you.',
    color: '#fbbf24',
  ...
  'yolo-deployer': {
    ...
    description: 'Straight to production. No fear. Pure chaos energy. May your deploys be ever green.',
    color: '#22d3ee',
...
export function determinePersona(bundle: {
  ...
  const { stats, traits, wordAnalysis, promptStats, topics, quirks } = bundle;
  ...
  // Caps lock commander - lots of ALL CAPS prompts
  if (promptStats?.allCapsPrompts && promptStats.allCapsPrompts > 10) {
  ...
  // Polite menace - very high politeness score
  if (wordAnalysis?.politenessScore && wordAnalysis.politenessScore > 70) {
    return 'polite-menace';
  }

  // Debug addict - lots of debugging keywords
  if (topics?.debugging && topics.debugging > 500) {
    return 'debug-addict';
  }

  // CSS Struggler - heavy frontend work
  if (topics?.frontend && topics.frontend > 400 && topics.frontend > (topics?.debugging || 0)) {
    return 'css-struggler';
  }

  // Essay writer - very long average prompts
  if (promptStats?.averageLength && promptStats.averageLength > 300) {
    return 'essay-writer';
  }

  // One word wonder - very short average prompts
  if (promptStats?.averageLength && promptStats.averageLength < 30) {
  ...
  // Vibe coder - heavy "fix" usage with low context
  if (wordAnalysis?.fixCount && wordAnalysis.fixCount > 200 &&
      wordAnalysis?.helpCount && wordAnalysis.helpCount > 150) {
    return 'vibe-coder';
  }
...
export function getQuirkRoast(quirk: string, value: number): string {
  const roasts: Record<string, (v: number) => string> = {
    ...
    'lateNightSessions': (v) => `${v} sessions after midnight. Sleep is for people without deadlines.`,
