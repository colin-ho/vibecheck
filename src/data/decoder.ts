...
async function enrichBundle(bundle: AnonymousBundle): Promise<UsageData> {
  ...
  const personaId = bundle.personaId || determinePersona({
    stats: bundle.stats,
    traits: bundle.traits,
    wordAnalysis: bundle.wordAnalysis,
    promptStats: bundle.promptStats,
...
function estimatePercentiles(bundle: AnonymousBundle): Percentiles {
  ...
  return {
    ...
    sessionLength: Math.max(1, Math.min(99, 100 - Math.log10(stats.longestSessionMinutes / 10) * 20)),
