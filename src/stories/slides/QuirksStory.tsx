...
export function QuirksStory({ data, isActive }: StorySlideProps) {
	const { quirks, stats, insights } = data
	...
	if (quirks?.interruptCount && quirks.interruptCount > 0) {
		crimes.push({
			...
			severity:
				quirks.interruptCount > 20 ? 'major' : quirks.interruptCount > 10 ? 'moderate' : 'minor',
			color: '#da1c1c',
		})
	}
	...
	if (quirks?.lateNightSessions && quirks.lateNightSessions > 0) {
		crimes.push({
			...
			severity:
				quirks.lateNightSessions > 50
					? 'major'
					: quirks.lateNightSessions > 20
						? 'moderate'
						: 'minor',
			color: '#bdb7fc',
		})
	...
	// Early morning sessions (before 6am)
	if (quirks?.earlyMorningSessions && quirks.earlyMorningSessions > 0) {
		crimes.push({
			icon: 'sunrise',
			title: 'Early Bird Sessions',
			value: quirks.earlyMorningSessions,
			roast:
				quirks.earlyMorningSessions > 20
					? "Either an early bird or you never went to bed. We're not judging."
					: 'Up before the sun. Your dedication is noted.',
			severity:
				quirks.earlyMorningSessions > 30
					? 'major'
					: quirks.earlyMorningSessions > 10
						? 'moderate'
						: 'minor',
			color: '#dd5013',
		})
	...
	if (quirks?.weekendPercentage && quirks.weekendPercentage > 20) {
		crimes.push({
			...
			severity:
				quirks.weekendPercentage > 40
					? 'major'
					: quirks.weekendPercentage > 30
						? 'moderate'
						: 'minor',
			color: '#dd5013',
		})
	...
	if (quirks?.longestStreakDays && quirks.longestStreakDays > 3) {
		crimes.push({
			...
			severity:
				quirks.longestStreakDays > 14
					? 'major'
					: quirks.longestStreakDays > 7
						? 'moderate'
						: 'minor',
			color: '#8b372b',
		})
	...
	if (quirks?.abandonedSessions && quirks.abandonedSessions > 5) {
		crimes.push({
			...
			severity:
				quirks.abandonedSessions > 30
					? 'major'
					: quirks.abandonedSessions > 15
						? 'moderate'
						: 'minor',
			color: '#5d3d3a',
		})
	...
	// Communication style based quirks
	...
	if (politeness === 'demanding') {
		crimes.push({
			...
			value: 'Demanding',
			roast: 'You know what you want. And you want it now.',
			severity: 'moderate',
			color: '#da1c1c',
		})
	} else if (politeness === 'apologetic') {
		crimes.push({
			icon: 'heart',
			title: 'Communication Style',
			value: 'Apologetic',
			roast: "You apologize to your code. That's... concerning.",
			severity: 'minor',
			color: '#bdb7fc',
		})
	}
	...
	// Contrast-based crimes
	const contrasts = insights?.contrasts

	// The Minimalist - shortest effective prompt
	if (contrasts?.shortestEffective && contrasts.shortestEffective.length <= 20) {
		crimes.push({
			icon: 'sparkle',
			title: 'The Minimalist',
			value: `"${contrasts.shortestEffective}"`,
			roast: 'Sometimes less is more. You proved it.',
			severity: 'minor',
			color: '#a05f1a',
		})
	}

	// The Diplomat - politest moment
	if (contrasts?.politestMoment) {
		const truncated =
			contrasts.politestMoment.length > 30
				? contrasts.politestMoment.slice(0, 30) + '...'
				: contrasts.politestMoment
		crimes.push({
			icon: 'heart',
			title: 'The Diplomat',
			value: `"${truncated}"`,
			roast: 'Claude appreciates the manners. Gold star.',
			severity: 'minor',
			color: '#bdb7fc',
		})
	}
	...
	// The Demanding One
	if (contrasts?.mostDemanding) {
		const truncated =
			contrasts.mostDemanding.length > 30
				? contrasts.mostDemanding.slice(0, 30) + '...'
				: contrasts.mostDemanding
		crimes.push({
			icon: 'bolt',
			title: 'Most Demanding',
			value: `"${truncated}"`,
			roast: 'You knew what you wanted. No time for pleasantries.',
			severity: 'moderate',
			color: '#da1c1c',
		})
	}
	...
	// Take top 6 if available
	const displayCrimes = crimes.slice(0, 6)
	...
	// Pre-generate stable random positions for floating indicators
	...
	return (
		<SlideLayout className="relative">
				...
				{displayCrimes.map((crime, index) => (
					...
					>
						...
						<div className="flex items-start gap-3 pl-2">
							{/* Severity indicator */}
							<div className="text-xs font-mono font-bold mt-1" style={{ color: crime.color }}>
			...
			<div className="absolute inset-0 pointer-events-none overflow-hidden">
				{displayCrimes.slice(0, 3).map((crime, index) => (
					<motion.div
						...
						transition={{
							duration: floatingPositions[index].duration,
							repeat: Infinity,
							delay: floatingPositions[index].delay,
						}}
					>
						{getSeverityIcon(crime.severity)}
					</motion.div>
