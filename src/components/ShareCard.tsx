...
export const ShareCard = forwardRef<HTMLDivElement, ShareCardProps>(({ data }, ref) => {
	const { stats, persona, percentiles, funFacts } = data
	...
	return (
		...
		>
			...
			<div className="flex-1 flex flex-col justify-center">
				<div className="text-6xl font-black text-white tracking-tight">{persona.name}</div>
				<div className="text-2xl text-white/80 mt-2 font-medium">{persona.tagline}</div>
				{funFact && <div className="text-lg text-white/60 mt-3 italic">{funFact}</div>}
			</div>

			{/* Stats Grid */}
			<div className="grid grid-cols-3 gap-8">
				<div>
					<div className="text-4xl font-bold text-white">{formatNumber(totalTokens)}</div>
