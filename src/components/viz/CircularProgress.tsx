		...
		<div className="relative" style={{ width: size, height: size }}>
			<svg viewBox={`0 0 ${size} ${size}`} className="w-full h-full -rotate-90">
				...
				<defs>
					...
					<linearGradient
						id={`progress-gradient-${color.replace('#', '')}`}
						x1="0%"
						y1="0%"
						x2="100%"
						y2="0%"
				...
				<motion.circle
					...
					style={{
						filter:
							glowIntensity !== 'none'
								? `drop-shadow(${glowStyles[glowIntensity]} ${color}80)`
								: undefined,
					}}
				/>
...
export function FillGauge({
	...
	return (
		<div className="flex flex-col items-center">
			...
			{label && (
				<div className="text-[0.65rem] font-semibold tracking-[0.15em] uppercase text-cocoa/60 mt-3">
					{label}
				</div>
			)}
		</div>
