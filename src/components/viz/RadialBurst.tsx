...
export function RadialBurst({
  ...
  const dots = useMemo(() => {
    ...
    for (let ring = 0; ring < rings && dotsPlaced < displayCount; ring++) {
      ...
      const dotsInRing = Math.min(
        Math.floor((ring + 1) * 8),
        displayCount - dotsPlaced
      );
    ...
    <div className="relative" style={{ width: size, height: size }}>
      ...
      {dots.map((dot, index) => (
        <motion.div
          ...
          animate={{ opacity: 0.6 + Math.random() * 0.4, scale: 1 }}
          transition={{
      ...
      {count > maxDots && (
        <motion.div
          className="absolute bottom-2 left-1/2 -translate-x-1/2 text-xs text-dark/50"
